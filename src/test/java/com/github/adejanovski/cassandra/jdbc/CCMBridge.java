...
public class CCMBridge {
  ...
  private static final Object SIMPLE_KEYSPACE = "simple_ks";
  ...
  private static CCMBridge create(String name, int nbNodes, String... options) {
    checkArgument(!"current".equals(name.toLowerCase()), "cluster can't be called \"current\"");
    CCMBridge bridge = new CCMBridge();
  ...
  static CCMBridge create(String name, int nbNodesDC1, int nbNodesDC2) {
    checkArgument(!"current".equals(name.toLowerCase()), "cluster can't be called \"current\"");
    CCMBridge bridge = new CCMBridge();
  ...
  static String ipOfNode(int nodeNumber) {
  ...
  public static abstract class PerClassSingleNodeCluster {
    ...
    static CCMBridge cassandraCluster;
    ...
    abstract Collection<String> getTableDefinitions();
  ...
  public static class CCMCluster {
    ...
    final CCMBridge cassandraCluster;

    private boolean erroredOut;

    static CCMCluster create(int nbNodes, Cluster.Builder builder) {
      if (nbNodes == 0) {
    ...
    static CCMCluster create(int nbNodesDC1, int nbNodesDC2, Cluster.Builder builder) {
      if (nbNodesDC1 == 0) {
        throw new IllegalArgumentException();
      }
